...
1. Ensure a folder named `data` exists (the server will create it automatically if missing).  
2. Update the `API_KEY` variable in `server.py` with a valid NewsAPI.org API key.  
3. Start the server by running `python server.py`. The server listens on port 9000 and handles multiple clients using threads.  
4. Run the client by executing `python client.py`. Enter your name to register, navigate the menus to search headlines or sources, view details of selected items, and quit to disconnect.  
5. All retrieved data is saved in `data/` with the format:  
...
This project demonstrates a fully functional Python client-server system with network communication, API integration, multithreading, organized code, and proper documentation. Through this project, **we gained hands-on experience in developing a networked application and managing real-time data from online sources, which strengthened our practical programming skills and teamwork.**
